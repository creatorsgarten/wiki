...
1. Check all XLR connections.
2. Check each wireless microphone individually.
3. Check the mixer input levels.
4. Check the main output to the JBL EON208P.
5. Check the GROUP OUT signal to the Audio Interface.
6. Confirm that the streaming/recording system is receiving audio correctly.
7. Confirm that no audio is being routed back into the mixer.
...
- Do not enable 48V phantom power.
- Do not use a 3.5mm-to-XLR connection.
- Do not connect both GROUP OUT L and GROUP OUT R to the Audio Interface.
- Do not route the Audio Interface or streaming computer output back into the mixer.
- The Audio Interface must be connected directly to the GROUP OUT of the Yamaha MG12XU.
- All audio connections should be tested during sound check before the event begins.